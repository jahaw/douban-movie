let MovieComment=require('../models/comment') // 电影评论模型
// 电影评论后台录入控制器
exports.save = function(req,res) {
    let _comment = req.body.comment;        					 // 获取post发送的数据
    // 如果存在cid说明是对评论人进行回复
    if(_comment.cid) {
        ...
        // 将用户评论创建新对象并保存
        let comment = new MovieComment(_comment);
        comment.save(function(err,comment) {
            if(err){
                console.log(err);
            }
